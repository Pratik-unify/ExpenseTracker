...
export default function AddExpense() {
  ...
  return (
    // brand-card and brand-border is color assigned to custom variable in themes in index.css 
    <div className="max-w-md mx-auto bg-brand-card border border-brand-border/60 rounded-[9px] p-6 shadow-xs">
      <div className="flex justify-between items-center border-b border-brand-border pb-3.5 mb-5">
        <h2 className="font-sans text-[16px] font-bold text-brand-ink">Add new expense</h2>
        <button
          onClick={() => navigate("/")}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex flex-col">
          ...
          <input
            ...
            value={formData.name}
            autoComplete="off"
            // this was added to remove the automatic password  from some of the extensions that I use 
            data-1p-ignore
            data-lpignore="true"
            placeholder="What was it for?"
            // focus - when the user is active inside this input , ring adds box shadow  
